...
export class EventService extends BaseService {
  ...
  constructor() {
    ...
    this.id$ = combineLatest([this.trackRouteParam('event_id'), albumService.album.data$]).pipe(
      map(([urlId, album]) => urlId ?? (album ? album.event_id : null))
    );

    // Fetch event details
    this.event = this.fetchOnChange(this.id$, id => this.fetchEvent(id), null, true);

    // Fetch this event's albums
    const albums = this.fetchOnChange(this.id$, id => this.fetchEventAlbums(id), []);
    this.albums = this.transformFetchedObject(albums, albums => albums.sort((a, b) => b.views - a.views), [], true);

    // Fetch the current watermark
    this.watermarkUrl = this.fetchOnChange(this.id$, id => this.fetchEventWatermark(id), null);
  }

  create(event: EventCreate): Observable<EventDetailed> {
    return this.http.post<EventDetailed>('/event/', event).pipe(tap(() => this.snackbar.open('Event created.')));
  }

  update(id: PhotoEvent['id'], event: EventUpdate): Observable<EventDetailed> {
    return this.http.put<EventDetailed>(`/event/${id}`, event).pipe(tap(() => this.snackbar.open('Event updated.')));
  }
  ...
  delete(id: PhotoEvent['id']): Observable<boolean> {
    return this.http
      .delete(`/event/${id}`)
      .pipe(map(() => true))
      .pipe(tap(() => this.snackbar.open('Event deleted.')));
  }
