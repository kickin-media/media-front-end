...
export class PhotoService extends BaseService {
  ...
  constructor() {
    ...
    this.photo = this.fetchOnChange(this.id$, id => this.fetchPhoto(id), null);
  }
  ...
  upload(albumId: Album['id'], files: File[]): Observable<PhotoUploadStatus> {
    ...
    return this.http.post<PhotoUpload[]>('/photo/', '', { params: { num_uploads: files.length } }).pipe(
      ...
      map(item => {
        // Upload to S3
        return this.s3.uploadPhoto(files[item.index], item.photos[item.index].pre_signed_url).pipe(
          // Add to the selected album
          switchMap(() => this.setPhotoAlbums(item.photos[item.index].photo_id, [albumId])),
