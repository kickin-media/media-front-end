...
export class EventPageComponent {
  ...
  protected albums$: Observable<Record<string, Album[]>>;
  ...
  constructor() {
    ...
    const albums$ = combineLatest([
      ...
      map(([albums, canManage, canUpload]) => {
    ...
    this.canDelete$ = this.eventService.albums.data$.pipe(map(albums => albums.length === 0));
  }
    ...
    this.eventService.event.data$
      .pipe(
        ...
        switchMap(event => {
          ...
          const dialogRef = this.dialog.open(AlbumDialogComponent, { data: { event } as EventDialogProps });
  ...
  protected getAlbumGroupIndices(groups: Record<string, Album[]>): string[] {
